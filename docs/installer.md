...
The backend contract separates descriptor enumeration, exact-device claiming, and `start_readonly`. Its policy forbids flash writes, reset, security bypass, reconnection and watchdog changes. Loading a DA into RAM remains an active operation. `mtk_usb.ExactUsbBackend` implements the contract experimentally; it is available through the Python API but remains disconnected from the installer CLI. Tests use fake USB descriptors and MTK protocol objects. The pin does not authenticate third-party dependencies, which still need a controlled, versioned runtime environment.
...
Startup uses one preloader handshake, guards the MT6580 chip check, and requires a successful security-configuration response. It refuses protected targets and BootROM sessions. It uploads only the supplied verified loader in stock/unpatched mode, disables reset, rediscovery, security-bypass and flash-write entry points, and does not call the generic `configure_da`. A missing DRAM configuration or any requested reconnect stops startup. No loader or per-unit preloader image is bundled or automatically selected.
...
Select and pin an approved HA100 loader, determine whether its DRAM initialization requires additional board data, and validate the existing backend on a dedicated test session. Keep writing unavailable in that milestone. The existing reader obtains chip/storage identifiers, capacity and the complete GPT from the session; none of this proves HA100 model identity by itself.
