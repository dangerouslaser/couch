...
class PublicInstallerTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / 'source.tar.gz').write_bytes(b'actual source archive fixture')
        self.attestation = dict(schema=1, kind='couch-public-os-build', source_commit='a'*40,
                                source_archive_sha256=hashlib.sha256(b'actual source archive fixture').hexdigest(), complete=True, private_inputs=False,
    ...
    def test_source_archive_and_builder_receipt_bytes_must_match_attestation(self):
        ...
        with self.assertRaises(ValueError):
            self.build()
        source.write_bytes(b'actual source archive fixture')
