...
while [ $i -le 12 ]; do
    printf '  '
    $ADB exec-out "dd if=$PART_RECOVERY bs=512 skip=$((MARKER_SECTOR+i)) count=1" 2>/dev/null \
        | tr -d '\0' | head -1
    i=$((i+1))
done
mkdir -p build
$ADB exec-out "dd if=$PART_RECOVERY bs=512 skip=$DMESG_SECTOR count=2048" 2>/dev/null \
    | tr -d '\0' > build/linux-dmesg.txt
$ADB exec-out "dd if=$PART_RECOVERY bs=512 skip=$DMESG2_SECTOR count=2048" 2>/dev/null \
