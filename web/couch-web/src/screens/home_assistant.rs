...
pub fn setup(app: App) -> AnyView {
    ...
    let discover = move |_| {
        ...
        spawn_local(async move {
            match api::ha("GET", "/api/ha/lights", None).await {
                Ok(result) => {
    ...
    view! {
        ...
        <p>"Connect your server, find its lights, then add them to rooms. Start with on/off and brightness."</p>
...
fn discovered(app: App, light: Value) -> AnyView {
    ...
    let already = {
        let id = id.clone();
        move || {
            app.config.get().is_some_and(|c| c.devices().any(|(_,d)| matches!(&d.integration,couch_model::Integration::HomeAssistant{entity_id} if entity_id == &id)))
        }
    };
    let add_id = id.clone();
    ...
    view! {
        ...
        <button class="ghost" disabled=move || app.busy.get() || room.get().is_empty() || already() on:click=move |_| {
            app.run(api::post(format!("/api/rooms/{}/devices",room.get_untracked()),json!({"name":add_name,"kind":"light","integration":{"via":"home-assistant","entity_id":add_id}})));
        }>"Add light"</button>
        </div>
    }.into_any()
}
