...
try{
 ...
 pressed=true;await card.getByRole('button',{name:'Pair bridge',exact:true}).click();await card.getByText('Connected and saved.',{exact:false}).waitFor();
 ...
 await card.getByRole('button',{name:'Turn off',exact:true}).click();await card.getByText('Off',{exact:true}).waitFor();
 await card.getByLabel('Brightness (%)',{exact:true}).fill('37');await card.getByRole('button',{name:'Apply brightness'}).click();await card.getByText('On · 37%',{exact:true}).waitFor();
 ...
 await card.getByLabel('Add to room',{exact:true}).selectOption({index:1});await card.getByRole('button',{name:'Add light',exact:true}).click();await page.getByRole('status').filter({hasText:/^Saved$/}).waitFor();
 const cfg=await(await fetch(`${base}/api/config`)).json();assert(cfg.rooms.some(r=>r.devices.some(d=>d.integration.via==='hue' && d.integration.light_id===id)));
